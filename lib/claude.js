...
import { recordAiUsage } from "./store.js";
...
async function callClaude(prompt, maxTokens = 1000, label = "unlabeled") {
...
async function callClaudeForJson(prompt, maxTokens, contextLabel) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const raw = await callClaude(prompt, maxTokens, contextLabel);
    lastRaw = raw;
    try {
...
export async function draftAutoPostReply(review, bizName, tone) {
  ...
  return callClaude(prompt, 300, "auto_post_reply");
}
...
export async function draftReply(review, bizName, tone) {
  ...
  const result = await callClaude(prompt, 1000, "draft_reply");
