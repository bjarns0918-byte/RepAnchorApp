...
function parseJsonResponse(raw, contextLabel) {
  let cleaned = raw.replace(/```json|```/g, "").trim();
  try {
    return JSON.parse(cleaned);
  } catch (err) {
    // Fall back to extracting whatever looks like the outermost JSON
    // object/array, in case there's stray text before or after it.
    ...
    throw new Error(`Claude's response for ${contextLabel} got cut off or wasn't valid. Please try again.`);
  }
}
...
export async function generateSampleReviews(businessName) {
  ...
  return parseJsonResponse(raw, "the sample reviews");
}
...
export async function extractThemes(reviews, bizName) {
  ...
  return parseJsonResponse(raw, "the sentiment themes");
}
...
export async function buildDigest(reviews, bizName) {
  ...
  const raw = await callClaude(prompt, 700);
  return parseJsonResponse(raw, "the digest");
}
