...
// Calls Claude and parses its JSON response, retrying once automatically if
// the first attempt comes back cut off or malformed - this alone resolves
// the large majority of occasional truncation issues.
async function callClaudeForJson(prompt, maxTokens, contextLabel) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      ...
      if (attempt === 2) {
        console.error(`[claude] Giving up parsing JSON for ${contextLabel} after 2 attempts. Last raw response was:\n${lastRaw}`);
        throw new Error(`Claude's response for ${contextLabel} got cut off or wasn't valid after retrying. Please try again.`);
      }
      console.warn(`[claude] JSON parse failed for ${contextLabel} on attempt ${attempt}, retrying once...`);
    }
  }
}
