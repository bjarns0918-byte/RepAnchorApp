...
// --- Automated backups ---
//
// Copies the current database to a timestamped file in data/backups/ and
// deletes anything older than 14 days, so a bad deploy or accidental data
// loss has a recent recovery point without needing an external service.
...
export function backupDatabase() {
  ...
  const backupPath = path.join(BACKUP_DIR, `db-${today}.json`);
...
// --- AI usage tracking (site-wide, not per-business) ---
//
// Since customer AI usage is intentionally uncapped, this gives visibility
// instead of a limit - so a real spike gets noticed early, not discovered
// on a bill.
export function recordAiUsage(functionName) {
  const db = readDb();
  if (!db.aiUsage) {
    db.aiUsage = { totalCalls: 0, byFunction: {}, since: new Date().toISOString() };
  }
  db.aiUsage.totalCalls += 1;
  db.aiUsage.byFunction[functionName] = (db.aiUsage.byFunction[functionName] || 0) + 1;
  writeDb(db);
}

export function getAiUsage() {
  const db = readDb();
  return db.aiUsage || { totalCalls: 0, byFunction: {}, since: new Date().toISOString() };
}
//
...
export function createBusiness({ email, passwordHash, businessName, phone, billingPlan }) {
  ...
  db.businesses[id] = {
    ...
    notifyUrgentReviews: true,
    digestFrequency: "monthly", // "weekly" or "monthly"
