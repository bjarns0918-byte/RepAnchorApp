...
import { generateTotpSecret, verifyTotpCode, buildOtpauthUri } from "./lib/totp.js";
...
function requireLogin(req, res, next) {
  ...
  req.isImpersonating = !!req.session.impersonatingAdminId;
  next();
}
...
app.post("/api/login", loginLimiter, (req, res) => {
  ...
  if (business && verifyPassword(password || "", business.passwordHash)) {
    if (business.twoFactorEnabled) {
      // Don't fully log in yet - just remember who's pending until they
      // provide a valid code from their authenticator app.
      req.session.pending2faBusinessId = business.id;
...
app.post("/api/login-2fa", loginLimiter, (req, res) => {
  const { code } = req.body;
  const pendingId = req.session.pending2faBusinessId;
  if (!pendingId) {
    return res.status(400).json({ ok: false, error: "No pending login found. Please log in again." });
  }
  const business = getBusinessById(pendingId);
  if (!business || !business.twoFactorSecret) {
    return res.status(400).json({ ok: false, error: "Something went wrong. Please log in again." });
  }
  if (!verifyTotpCode(business.twoFactorSecret, code)) {
    return res.status(400).json({ ok: false, error: "Incorrect code. Please check your authenticator app and try again." });
  }
  delete req.session.pending2faBusinessId;
  req.session.businessId = business.id;
  req.session.role = "owner";
  req.session.teamMemberId = null;
  res.json({ ok: true });
...
app.get("/api/admin/ai-usage", requireLogin, (req, res) => {
  if (!req.business.isAdmin) {
    return res.status(403).json({ ok: false, error: "Admin access only." });
  }
...
app.get("/api/admin/backups", requireLogin, (req, res) => {
  if (!req.business.isAdmin) {
    return res.status(403).json({ ok: false, error: "Admin access only." });
  }
...
// A small reusable guard for every /api/admin/business/:id route below.
function requireAdminAndFindBusiness(req, res) {
  if (!req.business.isAdmin) {
    res.status(403).json({ ok: false, error: "Admin access only." });
    return null;
  }
  const target = getBusinessById(req.params.id);
...
app.get("/api/admin/business/:id", requireLogin, (req, res) => {
  const target = requireAdminAndFindBusiness(req, res);
  if (!target) return;
  const reviews = getAllReviews(target.id);
  res.json({
    ok: true,
    business: {
      id: target.id,
      ...
      googleConnected: !!target.googleTokens,
      lastSyncedAt: target.lastSyncedAt,
      createdAt: target.createdAt,
      teamMembers: (target.teamMembers || []).map((t) => ({ email: t.email, invitedAt: t.invitedAt })),
      latestDigest: target.latestDigest,
      ratingHistory: target.ratingHistory || [],
      adminNotes: target.adminNotes || "",
      notifyUrgentReviews: target.notifyUrgentReviews !== false,
      digestFrequency: target.digestFrequency || "monthly"
    },
    reviews
  });
});

app.post("/api/admin/business/:id/notes", requireLogin, (req, res) => {
  const target = requireAdminAndFindBusiness(req, res);
  if (!target) return;
  updateBusiness(target.id, { adminNotes: (req.body.notes || "").slice(0, 5000) });
  res.json({ ok: true });
});

app.post("/api/admin/business/:id/impersonate", requireLogin, (req, res) => {
  const target = requireAdminAndFindBusiness(req, res);
  if (!target) return;
  req.session.impersonatingAdminId = req.business.id;
  req.session.businessId = target.id;
  req.session.role = "owner";
  req.session.teamMemberId = null;
  res.json({ ok: true });
});

app.post("/api/admin/stop-impersonate", requireLogin, (req, res) => {
  const adminId = req.session.impersonatingAdminId;
  if (!adminId) {
    return res.status(400).json({ ok: false, error: "You're not currently viewing as another account." });
  }
  req.session.businessId = adminId;
  req.session.role = "owner";
  req.session.teamMemberId = null;
...
app.post("/api/admin/business/:id/sync", requireLogin, async (req, res) => {
  const target = requireAdminAndFindBusiness(req, res);
  if (!target) return;
  if (!target.googleTokens) {
    return res.status(400).json({ ok: false, error: "This business hasn't connected Google yet." });
  }
  try {
    await syncOneBusiness(target);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
...
app.post("/api/admin/business/:id/digest", requireLogin, async (req, res) => {
  const target = requireAdminAndFindBusiness(req, res);
  if (!target) return;
  try {
    await runDigestForBusiness(target);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
...
app.post("/api/admin/business/:id/status", requireLogin, (req, res) => {
  const target = requireAdminAndFindBusiness(req, res);
  if (!target) return;
  const { status } = req.body;
  if (!["active", "pending", "canceled"].includes(status)) {
    return res.status(400).json({ ok: false, error: "Status must be active, pending, or canceled." });
  }
  updateBusiness(target.id, { subscriptionStatus: status });
  res.json({ ok: true });
});

app.post("/api/admin/business/:id/delete", requireLogin, async (req, res) => {
  const target = requireAdminAndFindBusiness(req, res);
  if (!target) return;
  try {
    await cancelSubscriptionImmediately(target);
...
app.get("/api/settings", requireLogin, (req, res) => {
  res.json({
    ...
    email: req.business.email,
    notifyUrgentReviews: req.business.notifyUrgentReviews !== false,
    digestFrequency: req.business.digestFrequency === "weekly" ? "weekly" : "monthly"
  });
});

app.post("/api/settings", requireLogin, requireOwner, (req, res) => {
  try {
    const { businessName, replyTone, phone, notifyUrgentReviews, digestFrequency } = req.body;
    ...
    updateBusiness(req.business.id, {
      ...
      notifyUrgentReviews: notifyUrgentReviews !== false,
      digestFrequency: digestFrequency === "weekly" ? "weekly" : "monthly"
    });
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
...
app.get("/api/2fa/status", requireLogin, (req, res) => {
  res.json({ ok: true, enabled: !!req.business.twoFactorEnabled });
});

app.post("/api/2fa/setup", requireLogin, requireOwner, (req, res) => {
  if (req.business.isAdmin) {
    return res.status(400).json({ ok: false, error: "The admin account can't set up 2FA here." });
  }
  const secret = generateTotpSecret();
  updateBusiness(req.business.id, { twoFactorPendingSecret: secret });
  const otpauthUri = buildOtpauthUri(secret, req.business.email);
  res.json({ ok: true, secret, otpauthUri });
});

app.post("/api/2fa/confirm", requireLogin, requireOwner, (req, res) => {
  const { code } = req.body;
  const pendingSecret = req.business.twoFactorPendingSecret;
  if (!pendingSecret) {
    return res.status(400).json({ ok: false, error: "No 2FA setup in progress. Please start over." });
  }
  if (!verifyTotpCode(pendingSecret, code)) {
    return res.status(400).json({ ok: false, error: "That code doesn't match. Check your authenticator app and try again." });
  }
  updateBusiness(req.business.id, {
...
app.post("/api/2fa/disable", requireLogin, requireOwner, (req, res) => {
...
app.get("/api/reviews-csv", requireLogin, (req, res) => {
  const reviews = getAllReviews(req.business.id);
  ...
  const header = ["Date", "Reviewer", "Stars", "Review", "Status", "Reply"];
  const rows = reviews.map((r) => [
    r.createTime ? new Date(r.createTime).toLocaleDateString("en-US") : "",
    r.reviewer || "",
    starValue[r.starRating] || "",
    r.comment || "",
    r.status === "posted" ? "Auto-posted" : r.status === "pending_approval" ? "Needs approval" : r.status || "",
    r.postedReply || (r.draftReplies && r.draftReplies[0]) || ""
  ]);

  const brandingLine = [`${req.business.businessName} reviews - exported from RepAnchor on ${new Date().toLocaleDateString("en-US")}`];
  const csv = [brandingLine, [], header, ...rows].map((row) => row.map(escapeCsv).join(",")).join("\n");

  res.setHeader("Content-Type", "text/csv");
  res.setHeader("Content-Disposition", `attachment; filename="${req.business.businessName.replace(/[^a-z0-9]/gi, "_")}_reviews.csv"`);
  res.send(csv);
});
  ...
  try {
    ...
    for (const result of replyResults) {
      ...
      if (result.kind === "posted") {
        ...
        drafted++;
        if (URGENT_ALERT_STARS.includes(result.starRating) && business.notifyUrgentReviews !== false) {
...
// A "wrapped"-style recap computed entirely from data already on file - no
// extra AI calls needed. Available once there's at least 2 periods of
// history to compare; genuinely "yearly" once 12+ exist.
app.get("/api/recap", requireLogin, (req, res) => {
  const history = req.business.ratingHistory || [];
  if (history.length < 2) {
  ...
  const best = history.reduce((a, b) => (b.avgRating > a.avgRating ? b : a));
  const totalReviews = history.reduce((sum, h) => sum + (h.reviewCount || 0), 0);
  const ratingChange = Math.round((latest.avgRating - first.avgRating) * 10) / 10;

  res.json({
    ok: true,
    available: true,
    isFullYear: history.length >= 12,
    periodCount: history.length,
    firstLabel: first.label,
    latestLabel: latest.label,
    firstRating: first.avgRating,
    latestRating: latest.avgRating,
    ratingChange,
    bestLabel: best.label,
    bestRating: best.avgRating,
    totalReviews
  });
});
...
app.get("/api/digest-pdf", requireLogin, (req, res) => {
  ...
  // Draws a filled 5-point star as an actual vector shape. Standard PDF
  // fonts (like Helvetica) don't support the "★" Unicode character - it
  // silently renders as the wrong glyph - so stars are drawn, not typed.
  ...
  // Logo mark - the exact same anchor-and-star-dots design used site-wide
  // (header, login, every page), just drawn as vector shapes here instead
  // of SVG, so it matches instead of being a simplified stand-in.
  doc.save();
  doc.translate(marginX + 12, 32);
  doc.circle(0, -9, 2.3).lineWidth(1.6).stroke(gold);
  doc.moveTo(0, -6.7).lineTo(0, 3).lineWidth(1.8).stroke(gold);
  doc.moveTo(-4.7, -0.5).lineTo(4.7, -0.5).lineWidth(1.8).stroke(gold);
  doc.moveTo(-8, 2).quadraticCurveTo(-8.5, 7, -2.5, 8).lineWidth(1.8).stroke(gold);
  doc.moveTo(8, 2).quadraticCurveTo(8.5, 7, 2.5, 8).lineWidth(1.8).stroke(gold);
  [-7, -3.5, 0, 3.5, 7].forEach((dx) => {
    doc.circle(dx, 12.3, 1).fill(gold);
  });
  doc.restore();

  doc.fillColor("#ffffff").fontSize(13).font("Helvetica-Bold").text("RepAnchor", marginX + 32, 24);

  doc.fillColor("#ffffff", 0.55).fontSize(9).font("Helvetica")
    .text(monthLabel.toUpperCase(), marginX, 64, { characterSpacing: 0.6 });
  doc.fillColor("#ffffff", 1).fontSize(19).font("Helvetica-Bold").text(business.businessName, marginX, 88);

  if (latest) {
    const ratingText = `${latest.avgRating}`;
    doc.fillColor(gold, 1).fontSize(28).font("Helvetica-Bold");
    const ratingWidth = doc.widthOfString(ratingText);
    const starSize = 11;
    const groupWidth = ratingWidth + 10 + starSize * 2;
    const groupRight = pageWidth - marginX;
    doc.text(ratingText, groupRight - groupWidth, 28, { lineBreak: false });
    drawStar(groupRight - starSize, 42, starSize, gold);

    doc.fillColor("#ffffff", 0.6).fontSize(9).font("Helvetica")
      .text(`${latest.reviewCount} reviews this period`, 0, 72, { align: "right", width: pageWidth - marginX });
  }
...
// Catches any unhandled server error so a customer sees a branded page
// instead of a raw stack trace or a blank connection failure. Express
// recognizes this as error-handling middleware specifically because it
// takes four arguments - that's required, not stylistic.
